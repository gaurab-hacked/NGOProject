...
export default function TablePage(props) {
  const { handelPostalbum, albumData, handelDelete, handelUpdate, postUpload } =
    props;
  ...
  const filteredItems = React.useMemo(() => {
    let filteredUsers = [...albumData];

    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((user) =>
        user.albumName.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
  ...
  return (
    <>
      ...
      >
        ...
        <TableBody emptyContent={"No album found"} items={sortedItems}>
          {(item) => (
