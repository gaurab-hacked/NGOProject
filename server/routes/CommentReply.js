...
route.post("/blog/commentreply", fetchUser, async (req, res) => {
  try {
    const { commentId, description, like, blogId } = req.body;
    ...
    const newCommnetrep = await new CommentReply({
      commentId,
      description,
      likeId: String(like).toLocaleLowerCase() === "true" ? userId : [],
      userId,
      blogId,
    });
    await newCommnetrep.save();
    ...
    res.json({ success: "Comment Success" });
  } catch (error) {
    res.status(500).json({ error: "Enternal Server Error " + error });
  }
});
